...
import com.atanana.ratingbot.mocks.{MockRequisitionsChecker, MockTournamentsChecker}
...
class MainCheckerTest extends AnyWordSpecLike with Matchers {
  ...
  "MainChecker" should {
    ...
    "return no new tournaments on first run" in {
      ...
      checker.check(data, ratingbot.data.ParsedData(newTournaments, Set.empty)).tournamentsCheckResult shouldEqual TournamentsCheckResult(Set.empty, Set(ChangedTournament(tournament, 4)))
    }
  }
}
